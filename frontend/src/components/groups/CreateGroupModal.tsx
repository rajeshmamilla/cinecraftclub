        ...
        <div className="p-6 space-y-6">
          {step === 1 ? (
            <div className="space-y-4">
              <label className="text-sm font-medium text-muted-foreground">First, select a movie or show</label>
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {isSearching ? (
                  <div className="p-4 text-center text-muted-foreground animate-pulse text-sm">Searching...</div>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setStep(2);
                      setFormData(prev => ({ ...prev, name: `${movie.title || movie.name} Discussion` }));
                    }}
                    className="w-full flex items-center space-x-3 p-2 hover:bg-secondary rounded-lg transition-colors text-left"
                  >
                    <img src={getImageUrl(movie.poster_path, 'w185')} alt="" className="w-10 h-14 object-cover rounded" />
                    <div>
                      <div className="font-medium text-sm">{movie.title || movie.name}</div>
                      <div className="text-xs text-muted-foreground">{movie.release_date?.split('-')[0] || movie.first_air_date?.split('-')[0]}</div>
                    </div>
                  </button>
                ))}
              </div>
            ...
            <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
              <div className="flex items-center space-x-3 p-3 bg-secondary/30 rounded-xl border border-border mb-4">
                <img src={getImageUrl(selectedMovie?.poster_path || null, 'w185')} alt="" className="w-12 h-16 object-cover rounded" />
                <div className="flex-1">
                  <div className="text-xs text-muted-foreground mb-1">Selected Movie</div>
                  <div className="font-bold text-sm">{selectedMovie?.title || selectedMovie?.name}</div>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between p-3 bg-secondary/20 rounded-lg border border-border">
                  <div className="flex items-center space-x-2">
                    {formData.isPrivate ? <Lock className="w-4 h-4 text-yellow-500" /> : <Globe className="w-4 h-4 text-blue-500" />}
                    <span className="text-sm font-medium">{formData.isPrivate ? "Private Group" : "Public Group"}</span>
                  </div>
                  <button
                    onClick={() => setFormData({ ...formData, isPrivate: !formData.isPrivate })}
                    className={`relative w-11 h-6 rounded-full transition-colors ${formData.isPrivate ? 'bg-primary' : 'bg-muted'}`}
                  >
                    <div className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform ${formData.isPrivate ? 'translate-x-5' : ''}`} />
                  </button>
                </div>
              </div>
