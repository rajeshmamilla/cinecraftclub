  ...
  if (hasError) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-300">
        ...
        <h2 className="text-2xl font-bold text-white mb-2">Network Connection Error</h2>
        <p className="text-muted-foreground max-w-md mb-8 text-sm">
          Failed to fetch movie details from TMDB. This can happen due to a temporary network change, a DNS issue, or because the connection is restricted by your local ISP or network provider.
        </p>
        <button
          onClick={() => {
            setReviewsTrigger(prev => prev + 1);
          }}
          className="bg-primary text-primary-foreground px-6 py-2.5 rounded-lg font-bold hover:bg-primary/95 transition-all hover:scale-105 cursor-pointer"
        >
          Try Again
        </button>
      </div>
